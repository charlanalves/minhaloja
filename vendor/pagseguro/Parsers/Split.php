...
namespace vendor\pagseguro\Parsers;

use vendor\pagseguro\Domains\Requests\Requests;

/**
 * Class Basic
 * @package vendor\pagseguro\Parsers
 */
trait Split
{
    /**
                ...
                foreach ($receivers as $key => $value) {
                    ...
                    if (!is_null($receivers[$key]->getAmount())) {
                        $data[sprintf($properties::RECEIVER_SPLIT_AMOUNT, $count)] =
                            \vendor\pagseguro\Helpers\Currency::toDecimal($receivers[$key]->getAmount());
                    }
                    if (!is_null($receivers[$key]->getRatePercent())) {
                        $data[sprintf($properties::RECEIVER_SPLIT_RATE_PERCENT, $count)] =
                            \vendor\pagseguro\Helpers\Currency::toDecimal($receivers[$key]->getRatePercent());
                    }
                    if (!is_null($receivers[$key]->getFeePercent())) {
                        $data[sprintf($properties::RECEIVER_SPLIT_FEE_PERCENT, $count)] =
                            \vendor\pagseguro\Helpers\Currency::toDecimal($receivers[$key]->getFeePercent());
                    }
                }
            }
        }
